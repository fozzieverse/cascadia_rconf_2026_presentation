...
colnames(benches) <- c('Package', 'Method', 'Data Size', 'Seconds', 'Peak Memory (MB)')
...
benches_long <- benches |> pivot_longer(
  ...
  mutate(
    ...
    `Data Size` = `Data Size` / 1e3,
)

p <- ggplot(benches_long, aes(x = factor(`Data Size`), y = value, fill = Package, group = Package)) +
  geom_col(position = position_dodge(width = 0.5), width = 0.5) +
  facet_wrap(metric ~ Method, scales = "free") +
  labs(x = "Dataframe Size (Thousands)", y = "Value", fill = "Package") +
